...
class CaptionGenerator(object):
    ...
    def beam_search(self, rnn_input, v_feat, initial_state=None):
        ...
        def get_topk_words(embeddings, state):
            ...
            output = self.classifier(output.squeeze(self.seq_dim))
            if not self.include_unknown:
                output[:, self.unk_id] = output.min(1)[0]
        ...
        for _ in range(self.max_caption_length - 1):
            ...
            embeddings = self.embedder(input_feed).view(len(input_feed), 1, -1)
            embeddings = torch.cat([embeddings, v_feat.expand(len(input_feed), 1, -1)], 2)
