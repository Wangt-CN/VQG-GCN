...
data_path = '/home/xiaojinhui/zjp_VQA/bottom-up-attention/data/genome/1600-400-20'
# Load classes
...
def load_image_ids(split_name):
    ...
    if split_name == 'coco_train2014':
      with open('/mnt/data/xiaojinhui/VQA/VQA/coco/annotations/annotations/captions_train2014.json') as f:
        data = json.load(f)
        for item in data['images']:
          image_id = int(item['id'])
          filepath = os.path.join('/mnt/data/linkaiyi/mscoco/train2014/', item['file_name'])
          split.append((filepath,image_id))
    if split_name == 'coco_val2014':
      with open('/mnt/data/xiaojinhui/VQA/VQA/coco/annotations/annotations/captions_val2014.json') as f:
        data = json.load(f)
        for item in data['images']:
          image_id = int(item['id'])
          filepath = os.path.join('/mnt/data/linkaiyi/mscoco/val2014/', item['file_name'])
          split.append((filepath,image_id))
      ...
      with open('/data/visualgenome/image_data.json') as f:
        for item in json.load(f):
          ...
          split.append((filepath,image_id))      
    elif split_name == 'visual7w_train':
      with open('/mnt/data/xiaojinhui/wangtan/visual7w/dataset_v7w_telling.json') as f:
          data = json.load(f)
          for item in data['images']:
            image_id = int(item['image_id'])
            filepath = os.path.join('/mnt/data/xiaojinhui/wangtan/visual7w/images/', item['filename'])
            split.append((filepath, image_id))
    else:
      print 'Unknown split'
    return split
